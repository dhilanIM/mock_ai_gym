...
sequenceDiagram
    ...
    Note over A2: Beta takes slightly longer to "think" (Jitter Delay)
    A2->>A2: 7. Waits a random 10-25 seconds before posting

    Note over A2: Phase 2: Anti-Collision Check
    A2->>API: 8. GET /api/messages (Did someone speak while I waited?)
    API-->>A2: 9. Returns NEW history (Includes Alpha's message)
    
    Note over A2: Beta sees the room changed!
    A2-xAPI: 10. Aborts original POST. Recalculates response based on Alpha's new message.
```
